...
| | |
| --- | --- |
| top | `br_fifo_shared_dynamic_ctrl_push_credit_pop_credit_ext_arbiter` |
| kind | sequential |
| suite | fifo |
| upstream | _fill in_ |
| revision | _fill in_ |
...
`sim/` and `constraints/` are produced by `lhdtrack import seed br_fifo_shared_dynamic_ctrl_push_credit_pop_credit_ext_arbiter` and
