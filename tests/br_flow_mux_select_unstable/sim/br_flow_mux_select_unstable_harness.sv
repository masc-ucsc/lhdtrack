...
//
// Wraps br_flow_mux_select_unstable with an LFSR stimulus generator and a rotate-xor checksum, so
// every simulator compiles and runs the SAME state machine. `checksum` is the
// only thing the driver reads.
`default_nettype none

module br_flow_mux_select_unstable_harness (
    input  wire        clk,
    input  wire        rst,
  ...
  br_flow_mux_select_unstable dut (
    .clk(clk),
    .rst(rst),
    .select(lfsr[1:0]),
    .push_valid(lfsr[5:2]),
    .push_data({70'd0, lfsr[63:6]}),
    .pop_ready(lfsr[6]),
